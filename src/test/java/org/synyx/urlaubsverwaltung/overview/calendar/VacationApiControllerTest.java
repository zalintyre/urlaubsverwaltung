...
class VacationApiControllerTest {
    ...
    void getVacations() throws Exception {
        ...
        when(applicationService.getApplicationsForACertainPeriodAndPersonAndState(any(LocalDate.class), any(LocalDate.class), eq(person), eq(ALLOWED_CANCEL_RE)))
    ...
    void getVacationsOfOthersOrDepartmentColleaguesWithoutDepartments() throws Exception {
        ...
        final Application vacationAllowedCancelRequested = createApplication(new Person("muster", "Muster", "Marlene", "muster@example.org"),
            of(2016, 5, 19), of(2016, 5, 20), FULL);
        vacationAllowed.setStatus(ALLOWED_CANCEL_RE);
        when(applicationService.getApplicationsForACertainPeriodAndState(any(LocalDate.class), any(LocalDate.class), eq(ALLOWED_CANCEL_RE)))
